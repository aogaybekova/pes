вот код который нужно переделать так, что сейчас код останавливает stop_walk робота при каждом движении в повороте и поворот не заканчивается, нужно, чтобы stop_walk  отрабатывал только после завершения поворота, а повторот завершался, если преграды больше не в близком диапазоне
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
