...
def comp_filter(angle, t, T):
    ...
    if (dena == 0):
        ...
        alpha = atan(acc['y']/dena)

    if (denb == 0):
        beta = 0
    else:
        beta = atan(acc['x']/denb)
...
"""PWM range setting """
for i in range(0,6):
    print('kit0: ', Spot.servo_table[i])
    kit0.servo[Spot.servo_table[i]].set_pulse_width_range(500, 2500)
for i in range(6,12):
    print('kit1: ', Spot.servo_table[i])
    kit1.servo[Spot.servo_table[i]].set_pulse_width_range(500, 2500)
